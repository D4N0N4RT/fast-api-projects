...
def short_url_generation(db: Session):
    for _ in range(10):
...
def shorten_url(item: URLCreate, db: Session = Depends(get_db)):
    ...
    #   existing = db.query(URLItem).filter(URLItem.short_id == short_id).first()
    #    if not existing:
    short_id = short_url_generation(db)
...
@app.put("/{short_id}")
def update_short_url(item: URLCreate, db: Session = Depends(get_db)):
    url_item = db.query(URLItem).filter(URLItem.full_url == item.url.__str__()).first()
    if not url_item:
        raise HTTPException(status_code=404, detail="Ссылка не найдена")
    short_id = short_url_generation(db)
    url_item.short_id = short_id
    db.commit()
    db.refresh(url_item)
    return {"short_url": f"{short_id}"}
